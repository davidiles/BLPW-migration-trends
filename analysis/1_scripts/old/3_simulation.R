...
sim_file <- "analysis/2_output/simulation/sim_results_nov17.Rdata"
...
for (simulation_rep in rev(1:100)){
  ...
  # ------------------
  slopes_true <- rnorm(2,0,0.05)
  X_sim <- matrix(0,nrow=jags_data$nstrata,ncol=jags_data$nyear)
  for (j in 1:jags_data$nstrata){
    X_sim[j,] <- exp(rnorm(jags_data$nyear,slopes_true[j] * ((1:jags_data$nyear)-1),0.1))
  }
  ...
  for (isotope_scenario in c(1)){
    ...
    out_refit$mcmc.info$elapsed.mins # about 4 min
    ...
    sim_results <- rbind(sim_results,data.frame(Simulation_Rep = simulation_rep,
                                                ...
                                                slope_est_q025 = out$q2.5$slope[1],
                                                slope_est_q50 = out$q50$slope[1],
                                                slope_est_q975 = out$q97.5$slope[1],
    ...
    sim_results <- rbind(sim_results,data.frame(Simulation_Rep = simulation_rep,
                                                ...
                                                slope_est_q025 = out$q2.5$slope[2],
                                                slope_est_q50 = out$q50$slope[2],
                                                slope_est_q975 = out$q97.5$slope[2],
