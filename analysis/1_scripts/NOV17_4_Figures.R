...
# Spring
load("analysis/2_output/Spring/analysis_Spring_revised2.RData")
...
# Fall
load("analysis/2_output/Fall/analysis_Fall_revised2.RData")
...
Figure1 <- plot_grid(map_Spring,map_Fall,nrow=2,labels=c("(a)","(b)"))

png(file ="analysis/2_output/Figures_Main_Text/Figure1.png", units = "in", width = 6, height = 10, res = 600)
Figure1
dev.off()

#------------------------------------------------
# Figure 1: Combined into a single map
#------------------------------------------------
...
png(file ="analysis/2_output/Figures_Main_Text/Figure1.png", units = "in", width = 8, height = 6, res = 600)
map_combined
dev.off()
...
#------------------------------------------------
# Figure 2: Regional population trajectories relative to 1998
#------------------------------------------------
  ...
  for (j in 1:jags_data_Spring$nstrata){
    ...
    regional_change_since_1998 <- rbind(regional_change_since_1998,
                                        data.frame(stratum_number = j,
                                                   ...
                                                   prob_large_decrease = mean(log_change<log(0.7)),
...
regional_change_since_1998$Stratum = c("East","West","Continental")[regional_change_since_1998$stratum_number] %>% factor(levels = c("West","East","Continental"))
regional_change_since_1998$Source <- factor(regional_change_since_1998$Source, levels = c("Pre-breeding migration","Post-breeding migration","Breeding Bird Survey"))

# ALSO CALCULATE 10-YEAR CHANGE (SINCE 2008)
regional_change_since_2008 <- data.frame()
year_vec <- 1998:2018
focal_year <- which(year_vec == 2008)
for (t in 1:jags_data_Spring$nyear){
  ...
  for (j in 1:jags_data_Spring$nstrata){
    ...
    log_change <- log(X_Spring[,j,focal_year]/X_Spring[,j,1])
    
    regional_change_since_2008 <- rbind(regional_change_since_2008,
                                        data.frame(stratum_number = j,
                                                   year_number = t,
                                                   Source = "Pre-breeding migration",
    ...
    log_change <- log(X_Fall[,j,focal_year]/X_Fall[,j,1])
    ...
    log_change <- log(X[,focal_year]/X[,1])
  ...
  log_change <- log(XSum_Spring[,focal_year]/XSum_Spring[,1])
  ...
  log_change <- log(XSum_Fall[,focal_year]/XSum_Fall[,1])
  ...
  log_change <- log(BBS_indices$samples$continental_Continental[,focal_year]/BBS_indices$samples$continental_Continental[,1])
...
regional_change_since_2008$Year = (2008:2018)[regional_change_since_2008$year_number]
regional_change_since_2008$Stratum = c("East","West","Continental")[regional_change_since_2008$stratum_number] %>% factor(levels = c("West","East","Continental"))
regional_change_since_2008$Source <- factor(regional_change_since_2008$Source, levels = c("Pre-breeding migration","Post-breeding migration","Breeding Bird Survey"))
...
png(file ="analysis/2_output/Figures_Main_Text/Figure2.png", units = "in", width = 8, height = 8, res = 600)
Figure2
dev.off()

#-------------------------------------------------------------------
# Summarize change/trend estimates from each program, for each region
#  - also calculate change since 2008
#-------------------------------------------------------------------

Trend_Summary_1998 <- regional_change_since_1998 %>%
  subset(Year == 2018) %>%
  mutate(Stratum = factor(Stratum, levels = c("West","East","Continental")),
         '% change since 1998' = paste0(round(100*(exp(log_change_q50)-1),1),
                                        " (", round(100*(exp(log_change_q025)-1),1),
                                        " to ",round(100*(exp(log_change_q975)-1),1),
                                        ")"),
         
         'Trend' = paste0(round(100*(exp(log_change_q50)^(1/(2018-1998))-1),1),
                          " (", round(100*(exp(log_change_q025)^(1/(2018-1998))-1),1),
                          " to ",round(100*(exp(log_change_q975)^(1/(2018-1998))-1),1),
                          ")"),
         'Prob(Decline)' = round(prob_decline,2),
         'Prob(>30% Decline)' = round(prob_large_decrease,2)
...
Trend_Summary_2008 <- regional_change_since_2008 %>%
  subset(Year == 2018) %>%
  mutate(Stratum = factor(Stratum, levels = c("West","East","Continental")),
         '% change since 2008' = paste0(round(100*(exp(log_change_q50)-1),1),
                                        " (", round(100*(exp(log_change_q025)-1),1),
                                        " to ",round(100*(exp(log_change_q975)-1),1),
                                        ")"),
         
         'Trend' = paste0(round(100*(exp(log_change_q50)^(1/(2018-2008))-1),1),
                          " (", round(100*(exp(log_change_q025)^(1/(2018-2008))-1),1),
                          " to ",round(100*(exp(log_change_q975)^(1/(2018-2008))-1),1),
                          ")"),
         'Prob(Decline)' = round(prob_decline,2),
         'Prob(>30% Decline)' = round(prob_large_decrease,2)
...
Trend_Summary <- left_join(Trend_Summary_1998 %>% dplyr::select(-5,-6),Trend_Summary_2008 %>% 
                             dplyr::select(Stratum,Source,'% change since 2008'))

write.csv(Trend_Summary, file = "analysis/2_output/Figures_Main_Text/Table2_Trend_Summary.csv",row.names = FALSE)
