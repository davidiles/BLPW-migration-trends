...
#             parallel = TRUE)
# 
# out$mcmc.info$elapsed.mins # 43 mins
...
station_order <- station_summary$`Station code`
# ***************************************************************
# ***************************************************************
...
r_df <- r_array %>%
  ...
  dplyr::rename(samp = Var1, stratum_number = Var2, year_number = Var3, X = value) %>%
  group_by(stratum_number,year_number) %>%
  summarize(q50 = quantile(X,0.5),
            q05 = quantile(X,0.05),
            q95 = quantile(X,0.95))

r_df$Year = year_vec[r_df$year_number]
r_df$Stratum = c("East","West")[r_df$stratum_number] %>% factor(levels = c("West","East"))

# ggplot(data = r_df, aes(x = Year, y = q50,ymin=q05,ymax=q95, fill = Stratum, col = Stratum))+
