...
# Analysis of Post-Preeding ("Fall") migration data
...
daily_count_plot <- ggplot(subset(count_df, station %in% c("LPBO")))+
  ...
  facet_wrap(year_abs~.)+
  ggtitle("Post-breeding migration data at LPBO")
...
out$mcmc.info$elapsed.mins # should take about 3 hours
...
plot_obs_vs_expected <- ggplot(data = expected_vs_observed)+
  ...
  geom_text(data = cor_obs_expected, aes(x = 1998, y = max*1.2, label = cor_label), hjust = 0, size = 3, col = "transparent")+
  facet_wrap(station~., scales = "free_y")+
